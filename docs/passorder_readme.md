...
6. [대시보드 시각화 프리뷰](#6-대시보드-시각화-프리뷰)
7. [프로젝트 실행 방법 및 구조](#7-프로젝트-실행-방법-및-구조)
8. [데이터 분석가 취준생의 회고 및 배운 점](#8-데이터-분석가-취준생의-회고-및-배운-점)
...
* **비즈니스 목적**:
  - BDAI 학회의 '데이터 분석 프로젝트 with AI' 최종 과제로, 실제 스타트업 비즈니스 모델의 위기 요인을 분석하고 데이터 기반의 실질적 해결책을 도출합니다.
...
### 4.1. 사용 데이터셋 (Dataset)
실제 기업 원천 데이터의 접근 한계를 극복하기 위해, 패스오더 앱의 실제 UI 플로우(스마트 오더 탐색 → 장바구니 → 결제)와 비즈니스 도메인 지식을 투영하여 AI(Google Antigravity)와 함께 **실제 서비스 수준의 정밀한 행동 로그(Event Log)**를 설계·생성했습니다.

* 데이터 규모: 90일간 2,500명의 유저 행동 로그 약 15만 건
* 주요 컬럼 스키마:
...
### 4.3. 대시보드 구축 툴 및 기술 스택
* **데이터 프로세싱**: `Python 3.10+`, `pandas`, `numpy`
* **시각화 및 프론트엔드**: Vanilla HTML5, CSS3 Custom Tokens, `Chart.js 4.4+`, `chartjs-plugin-datalabels`
* **데이터베이스 및 교차 검증**: `MySQL 8.0`, `DBeaver Community`
* **개발 및 구조화 환경**: Google Antigravity, VS Code
...
대시보드 상단에 배치된 5대 핵심 KPI 결과는 다음과 같습니다:
...
```markdown
![패스오더 대시보드 메인 뷰](../templates/passorder_tone_manner.png)
```
...
## 8. 데이터 분석가 취준생의 회고 및 배운 점

본 프로젝트는 데이터 분석가 취업을 준비하며 진행한 첫 번째 엔드투엔드(End-to-End) 프로덕트 분석 프로젝트였습니다. 단순한 툴 다루기를 넘어 다음과 같은 값진 배움을 얻었습니다:

### 1. 기업과 비즈니스 모델에 대한 깊은 이해가 먼저다
* 초기에 구글 플레이 스토어 리뷰 340건을 크롤링했을 때는 부정 리뷰가 5건에 불과해 문제 정의에 난항을 겪었습니다.
* 하지만 "메가커피와 컴포즈커피가 왜 패스오더 입점을 허락했을까?", "체리피커들이 첫 주문 후 어디로 사라질까?"라는 **비즈니스적 본질에 질문을 던지면서** 비로소 '프랜차이즈 자사 앱으로의 고객 유출'이라는 진짜 문제를 발견할 수 있었습니다.
...
* 아래와 같이 직접 작성한 SQL 쿼리 결과와 대시보드의 5대 KPI 수치가 100% 일치함을 확인했을 때, 데이터 분석가로서 가장 큰 희열과 데이터에 대한 확신을 가질 수 있었습니다.
